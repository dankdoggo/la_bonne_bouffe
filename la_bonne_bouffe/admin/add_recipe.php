...
if(!empty($_POST)){
	...
	if(empty($post['title-take']) || !minAndMaxLength($post['title-take'], 5, 25)){
			$errors[] = 'Merci d\'indiquer un titre de recette entre cinq et vignt-cinq caractères';
	}
	...
	if(!is_uploaded_file($_FILES['picture-take']['tmp_name']) || !file_exists($_FILES['picture-take']['tmp_name'])){ // ici on sécurise en doublant l'info: uploadé et existant
		...
		if(in_array($mimetype, $mimeTypeAllow)){ // si dans le tableau mimeTypeAllow la variable $mimetype existe
			...
			$photoName = uniqid('photo_produit_'); // on crée une variable pour renommer le fichier avec un ID unique : uniqid ('avec extension photo_produit_ avant')
